...
/**
 * The nine types the API can store. `FORMULA`, `ROLLUP`, `RELATION`, `FILES`
 * and `PERSON` are deliberately absent — each is blocked on a specific missing
 * thing (an expression evaluator, a second entity, object storage, a member
 * picker), and offering one in the picker would offer something that cannot
 * survive a reload.
 */
